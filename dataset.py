...
data = pd.read_csv(DATA_FILE,sep='\t', names=columns, header=1)
...
def save_images(images, species, directory='train', csv_name='temp.csv', augment=False):
    ...
    for index in range(len(images['original'])):
        image = utils.load_image_and_preprocess(
            images['original'][index], images['segmented'][index], resolution = 224)
        if type(image) != type([]):
...
def save_images_64(images, species, directory='train', csv_name='temp.csv', augment=True):
    cropped_images = []
    image_species = []
    image_paths = []
    count = 1
...
save_images(images_train, species_train, directory='train_224_svm',
            csv_name='leafsnap-dataset-train-images_224_2.csv', augment=False)
save_images(images_test, species_test, directory='test_224_svm',
            csv_name='leafsnap-dataset-test-images_224_svm.csv', augment=False)


save_images_64(images_train, species_train, directory='train',
            csv_name='leafsnap-dataset-train-images.csv', augment=False)
save_images_64(images_test, species_test, directory='test',
